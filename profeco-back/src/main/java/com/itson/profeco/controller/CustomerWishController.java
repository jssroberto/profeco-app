...
@Tag(name = "Customer Wish Management",
        description = "Operations for Customers on their own Wishes")
...
public class CustomerWishController {
    ...
    @Operation(summary = "Get all wishes for current customer",
            description = "Retrieves a list of all wishes belonging to the currently authenticated customer.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved wish list"),
            @ApiResponse(responseCode = "401",
                    description = "Unauthorized - Invalid or missing token"),
            @ApiResponse(responseCode = "403",
                    description = "Forbidden - User does not have CUSTOMER role or authentication issue")})
    @GetMapping
    ...
    @Operation(summary = "Get a specific wish by ID for current customer",
            description = "Retrieves a specific wish by its ID, only if it belongs to the currently authenticated customer.")
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404",
                    description = "Wish not found or does not belong to the user (handled by service)")})
    @GetMapping("/{id}")
    public ResponseEntity<WishResponse> getCurrentCustomerWishById(@Parameter(
            description = "ID of the wish to retrieve", required = true) @PathVariable UUID id) {
        UUID customerId = getCurrentCustomerId();
        WishResponse wish = wishService.getWishByIdForCurrentUser(id, customerId);
        return ResponseEntity.ok(wish);
    }

    @Operation(summary = "Create a new wish for current customer",
            description = "Adds a new wish associated with the currently authenticated customer.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Wish created successfully",
                    headers = @io.swagger.v3.oas.annotations.headers.Header(name = "Location",
                            description = "URL of the newly created wish")),
            ...
            @ApiResponse(responseCode = "403",
                    description = "Forbidden - User does not have CUSTOMER role"),
            @ApiResponse(responseCode = "404",
                    description = "Related Store specified in the request not found (handled by service)")})
    @PostMapping
    ...
    @Operation(summary = "Update an existing wish for current customer",
            description = "Updates the details of an existing wish, only if it belongs to the currently authenticated customer.")
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "403",
                    description = "Forbidden - User does not have CUSTOMER role"),
            @ApiResponse(responseCode = "404",
                    description = "Wish not found (or does not belong to user), or related Store not found (handled by service)")})
    @PutMapping("/{id}")
    public ResponseEntity<WishResponse> updateCustomerWish(
            @Parameter(description = "ID of the wish to update",
    ...
    @Operation(summary = "Delete a wish for current customer",
            description = "Deletes a wish by its ID, only if it belongs to the currently authenticated customer.")
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404",
                    description = "Wish not found or does not belong to the user (handled by service)")})
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteCustomerWish(@Parameter(
            description = "ID of the wish to delete", required = true) @PathVariable UUID id) {
