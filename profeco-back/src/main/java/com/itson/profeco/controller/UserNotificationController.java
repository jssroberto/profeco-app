...
public class UserNotificationController {
    ...
    private UserEntity getAuthenticatedUserEntity() {
        ...
        if (!(authentication.getPrincipal() instanceof CustomUserDetails)) {
            throw new IllegalStateException(
                    "Unexpected authentication principal type. Expected CustomUserDetails.");
        }

        UUID userId = ((CustomUserDetails) authentication.getPrincipal()).getGenericUserId();
        return userRepository.findById(userId).orElseThrow(() -> new EntityNotFoundException(
                "Authenticated user not found in database with ID: " + userId));
    }

    @Operation(summary = "Get all my notifications",
            description = "Returns all notifications for the currently authenticated customer.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<List<NotificationResponse>> getAllMyNotifications() {
        ...
        List<NotificationResponse> notifications =
                userNotificationService.getAllNotificationsForUser(userId);
        return ResponseEntity.ok(notifications);
    }

    @Operation(summary = "Get my active notifications",
            description = "Returns only active notifications for the authenticated customer.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200",
    ...
    public ResponseEntity<List<NotificationResponse>> getMyActiveNotifications() {
        UUID userId = getAuthenticatedUserEntity().getId();
        List<NotificationResponse> activeNotifications =
                userNotificationService.getActiveNotificationsForUser(userId);
    ...
    public ResponseEntity<UnreadNotificationsCountResponse> getMyUnreadNotificationsCount() {
        UUID userId = getAuthenticatedUserEntity().getId();
        UnreadNotificationsCountResponse countResponse =
                userNotificationService.getUnreadNotificationsCount(userId);
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "Unauthorized"),
            @ApiResponse(responseCode = "403",
                    description = "Forbidden - User does not have required role or does not own the notification"),
    ...
    public ResponseEntity<NotificationResponse> markNotificationAsRead(
            @Parameter(description = "ID of the notification to mark as read",
                    required = true) @PathVariable UUID notificationId) {
        UUID userId = getAuthenticatedUserEntity().getId();
        NotificationResponse updatedNotification =
