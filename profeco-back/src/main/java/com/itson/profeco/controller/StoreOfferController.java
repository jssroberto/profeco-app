...
public class StoreOfferController {
    ...
    public ResponseEntity<StoreOfferResponse> applyOrUpdateOffer(@Parameter(
            description = "ID of the store product to apply the offer to") @PathVariable UUID storeProductId,
            @Valid @RequestBody StoreOfferRequest request) {
        StoreOfferResponse response =
                storeProductService.applyOrUpdateOffer(storeProductId, request);
