...
public class StoreOfferController {
    ...
    @Operation(summary = "Apply or update an offer for a store product",
            description = "Applies a new offer or updates an existing one for a specific store product. The request body should contain offer details.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Offer applied/updated successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = StoreOfferResponse.class))),
    ...
    public ResponseEntity<StoreOfferResponse> applyOrUpdateOffer(
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Offer removed successfully, returning the product state without offer",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = StoreOfferResponse.class))),
            @ApiResponse(responseCode = "401", description = "Unauthorized"),
            @ApiResponse(responseCode = "403", description = "Forbidden - User does not have required permissions"),
            @ApiResponse(responseCode = "404", description = "Store product not found")
    ...
    public ResponseEntity<List<StoreOfferResponse>> getOffersActiveBetweenDates(
        ...
        return ResponseEntity.ok(storeProductService.getOffersActiveBetweenDates(startDate, endDate));
    }
    ...
    public ResponseEntity<List<StoreOfferResponse>> findCurrentProductOffers(
        ...
        return ResponseEntity.ok(storeProductService.findCurrentProductOffers(productId, currentDate));
    }
}